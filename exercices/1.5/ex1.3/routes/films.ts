...
router.get("/", (req, res) => {
  ...
  if (isNaN(minDuration) || minDuration <= 0) {
    res.sendStatus(400); // bad practice (will be improved in exercise 1.5)
  }
...
router.get("/:id", (req, res) => {
  ...
  if (isNaN(id)) {
    return res.sendStatus(400); // bad practice (will be improved in exercise 1.5)
  }

  const film = films.find((film) => film.id === id);

  if (film === undefined) {
    return res.sendStatus(404); // bad practice (will be improved in exercise 1.5)
  }
...
router.post("/", (req, res) => {
  ...
  const existingFilms = films.find((films) => ( films.title.toLowerCase === newFilm.title.toLowerCase && films.director.toLowerCase === newFilm.director.toLowerCase))
